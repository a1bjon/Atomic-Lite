...
def main():
    ...
    class App():
        ...
        def __init__(self, master):
            ...
            self.syntax_btn = tk.Button(master, text='syn', command=self.syntax)
            self.syntax_btn.place(x=600, y=0)
        ...
        def syntax(self):
            try:
                langs = ['py', 'java', 'c', 'cpp', 'js', 'ts', 'sh', 'cs', 'php', 'html', 'css']
                file = self.file_name_lbl['text'].split('|')[0].strip()
                file_ext = file.split('.')[1]
                if file_ext in langs:
                    cdg = ic.ColorDelegator()
                    cdg.prog = re.compile(r'\b(?P<MYGROUP>tkinter)\b|' + ic.make_pat().pattern, re.S)
                    cdg.idprog = re.compile(r'\s+(\w+)', re.S)
                    cdg.tagdefs['MYGROUP'] = {'foreground': '#7F7F7F', 'background': 'grey12'}
                    cdg.tagdefs['COMMENT'] = {'foreground': '#404040', 'background': 'grey12'}
                    cdg.tagdefs['KEYWORD'] = {'foreground': '#c657ff', 'background': 'grey12'}
                    cdg.tagdefs['BUILTIN'] = {'foreground': '#00b8a6', 'background': 'grey12'}
                    cdg.tagdefs['STRING'] = {'foreground': '#85b300', 'background': 'grey12'}
                    cdg.tagdefs['DEFINITION'] = {'foreground': '#1ac6ff', 'background': 'grey12'}
                    ip.Percolator(self.text_box).insertfilter(cdg)

                else:
                    return tk.messagebox.showerror(
                        'Syntax Error', 'Current file does not support syntax highlighting')

            except IndexError:
                return tk.messagebox.showerror(
                 'Syntax Error', 'Cannot add syntax highlighting as either no file present in environment or file might not have a file extension')
    ...
    print(f'Total Keystrokes: [{colorama.Fore.MAGENTA}{colorama.Style.BRIGHT}{int(len(App.events_lst)/2)}{colorama.Style.RESET_ALL}]\n')
